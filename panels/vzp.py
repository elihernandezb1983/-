...
def find_map_images(map_name: str) -> list[Path]:
    """Файлы карты из словаря VZP_MAPS → foro_vzp/."""
    filenames = config.VZP_MAPS.get(map_name, [])
    paths: list[Path] = []
    for name in filenames:
        path = config.FOTO_VZP_DIR / name
        if path.is_file():
            paths.append(path)
    return paths
...
class VzpMapsPanelView(ui.LayoutView):
    ...
    def __init__(self, panel_cfg: dict) -> None:
        super().__init__(timeout=None)
        self._panel_cfg = panel_cfg

        container = ui.Container(
            accent_color=discord.Colour(panel_cfg.get("accent_color", 0x5865F2)),
        )
    ...
    async def _reset_select(self, interaction: discord.Interaction) -> None:
        ...
        try:
            await message.edit(view=VzpMapsPanelView(self._panel_cfg))
    ...
    async def _on_select(self, interaction: discord.Interaction) -> None:
        ...
        if not image_paths:
            await interaction.followup.send(
                config.MESSAGES["vzp_map_no_images"].format(
                    map=map_name,
                    folder=config.FOTO_VZP_DIR.name,
                    files=", ".join(expected),
                ),
                ephemeral=True,
            )
        ...
        if len(image_paths) < len(expected):
            missing = [f for f in expected if not (config.FOTO_VZP_DIR / f).is_file()]
            await interaction.followup.send(
                config.MESSAGES["vzp_map_no_images"].format(
                    map=map_name,
                    folder=config.FOTO_VZP_DIR.name,
